...
from models.GTE_model_GRU import GTEProgramClassification
...
class GTESetup:
    ...
    def init_logger(self, log_dir, if_train):
        self.if_train = if_train
        self.run_id = datetime.now().strftime("%Y-%m-%d--%H-%M-%S")
        log_path = Path(log_dir)
        if not log_path.exists():
            log_path.mkdir(parents=True)
        self.logger = Logger("ProgramClassification_GRU_type", str(log_path / f"{self.run_id}_log.txt"))
    ...
    def init_model(self, hidden_dim, n_classes, dropout, bias, n_layers, checkpoint):
        self.logger.info("model setup...")
        self.logger.info("\n" + pformat(ex.current_run.config["model_setup"]))
        self.model = GTEProgramClassification(hidden_dim, self.vocab_size, n_classes, dropout, bias, n_layers)
    ...
    def train(
        ...
        warmup_steps = 0 if self.checkpoint else 0.1 * max_epoch
        self.scheduler = get_linear_schedule_with_warmup(self.optimizer, num_warmup_steps=warmup_steps, num_training_steps=max_epoch)
