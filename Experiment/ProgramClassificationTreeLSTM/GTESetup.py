...
from models.GTE_model_TreeLSTM import GTEProgramClassification
...
class GTESetup:
    ...
    def init_logger(self, log_dir, if_train):
        ...
        self.logger = Logger("ProgramClassification_TreeLSTM_type", str(log_path / f"{self.run_id}_log.txt"))
    ...
    def init_model(self, hidden_dim, n_classes, checkpoint):
        self.logger.info("model setup...")
        self.logger.info("\n" + pformat(ex.current_run.config["model_setup"]))
        self.model = GTEProgramClassification(hidden_dim, self.vocab_size, n_classes)
