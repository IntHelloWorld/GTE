...
from models.GTE_model_Transformer_absolute_pos import GTEProgramClassification
...
class GTESetup:
    ...
    def init_logger(self, log_dir, if_train):
        ...
        self.logger = Logger("ProgramClassification_Transformer_absolute_pos", str(log_path / f"{self.run_id}_log.txt"))
    ...
    def init_model(self, hidden_dim, num_heads, n_layers, dropout, bias, n_classes, checkpoint):
        self.logger.info("model setup...")
        self.logger.info("\n" + pformat(ex.current_run.config["model_setup"]))
        self.model = GTEProgramClassification(hidden_dim, num_heads, self.vocab_size, n_classes, n_layers, dropout, bias)
    ...
    def train(
        ...
        if self.checkpoint:
            self.scheduler = get_linear_schedule_with_warmup(self.optimizer, num_warmup_steps=0, num_training_steps=max_epoch)
        else:
